...
- **Extraction: The LLM Pass** — A single combined prompt extracts entities and relationships together. The prompt is rendered from `domain_spec.py` as the single source of truth and has three injected sections:
  - **Entity types** — the closed list of extractable types (Disease, Gene, Drug, Protein, Hormone, Enzyme, Biomarker, Symptom, Procedure, Mutation, Pathway, etc.) injected by name; `metadata_only` types like Paper, Author, Institution are excluded since they come from document metadata, not extraction.
  - **Predicates** — the full predicate list (TREATS, INHIBITS, CAUSES, ENCODES, INCREASES_RISK, ASSOCIATED_WITH, etc.) injected by name; `domain_spec.py` also carries domain/range constraints and `specificity` scores per predicate, which the prompt instructions use to guide the model toward more specific predicates over generic ones like `ASSOCIATED_WITH`.
  - **Vocabulary** — up to 500 entities already identified across the corpus, injected as preferred names so the model uses consistent terminology rather than creating new surface variants.
  - **The structural constraint that links entities to relationships** — the prompt instructs the model that every relationship `subject` and `object` must be the `id` of an entry in the `entities` array output. Entities must be extracted first (within the same response) before they can be referenced in relationships. This is the key closed-world constraint: the model cannot hallucinate a relationship whose participants were not also extracted and typed.
  - **Linguistic trust** — each relationship carries `linguistic_trust`: `asserted`, `suggested`, or `speculative`, capturing the hedging level of the source text as part of provenance.
  - **A second, separate prompt** extracts study design metadata (RCT, observational, sample size, etc.) from the Methods section; this is a lightweight call kept separate to avoid inflating the main extraction prompt.
