...
- **Extraction: The LLM Pass** — At minimum one prompt per document; staging into multiple focused calls is often worth the added complexity. The prompt(s) must be grounded in the schema: entity types and predicates injected from `domain_spec.py` so the model works within the closed vocabulary, not against an open-ended instruction to "find relationships."
  - **What must go into every extraction prompt** — the closed entity-type list (so the model classifies into known types only); the predicate list with domain/range guidance (so it reaches for specific predicates rather than defaulting to generic ones like `ASSOCIATED_WITH`); corpus vocabulary as preferred names (so surface variation is suppressed before it reaches deduplication); domain instructions covering classification edge cases and output format.
  - **The structural constraint that enforces closed-world extraction** — require that every relationship subject and object be an ID from the entities extracted in the same response. This makes it structurally impossible for the model to assert a relationship involving a participant it didn't also classify and type.
  - **Staging decisions** — combining entity and relationship extraction in one call is simpler and sufficient for many corpora; splitting them (entities first, then relationships over the resolved entity set) reduces hallucination surface area at the cost of latency and added pipeline complexity. Additional lightweight calls for ancillary metadata (study design, author affiliations) are better kept separate to avoid inflating the main extraction prompt.
  - **What the extraction output must contain** — per entity: a local ID, entity type (from the closed list), surface name, synonyms, and any authority ID hints the model can supply; per relationship: subject ID, predicate, object ID, evidence span ID, confidence, and linguistic trust (`asserted` / `suggested` / `speculative`); per evidence span: the source passage text, section, and paragraph index. This is the contract downstream stages depend on.
  - **Example prompt** — Appendix B shows an abstracted version of the medlit extraction prompt template, illustrating how entity types, predicates, and vocabulary are injected from the domain spec into a Jinja2 template. Readers can use it as a starting point once they have their own schema defined.
...
- **Extraction Prompt Template** — Abstracted Jinja2 template showing the three injection points (`{{ entity_types }}`, `{{ predicates }}`, `{{ vocab_section }}`), the structural constraint requiring relationship subjects/objects to be entity IDs from the same response, linguistic trust classification, evidence ID format, and the domain instructions block. The medlit domain instructions are shown as a worked example of how to encode entity classification edge cases, predicate guidance, and output format rules for a specific domain.
